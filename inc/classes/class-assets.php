...
class Assets {
	...
	public function register_assets() {
		...
		$this->register_style( 'doppio-coffee-styles', 'css/styles.css' );
	}
	...
	public function get_asset_meta( $file, $deps = [], $ver = false ) {
		$asset_meta_file = sprintf( '%s/js/%s.asset.php', untrailingslashit( DOPPIO_COFFEE_BUILD_DIR ), basename( $file, '.' . pathinfo( $file )['extension'] ) );
	...
	public function register_script( $handle, $file, $deps = [], $ver = false, $in_footer = true ) {
		...
		$file_path = sprintf( '%s/%s', DOPPIO_COFFEE_BUILD_DIR, $file );

		if ( ! \file_exists( $file_path ) ) {
			return false;
		}

		$src        = sprintf( DOPPIO_COFFEE_BUILD_URI . '/%s', $file );
	...
	public function enqueue_assets() {
		wp_enqueue_style( 'doppio-coffee-styles' );
	}
